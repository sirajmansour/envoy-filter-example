...
KinesisFilter::KinesisFilter(KinesisFilterConfigSharedPtr config) : config_(config) {
  kinesis_client_ = Aws::MakeShared<Aws::Kinesis::KinesisClient>(Aws::External::tinyxml2::ALLOCATION_TAG,Aws::Auth::AWSCredentials((config.get())->awsAccessKeyId(),(config.get())->awsAccessKeySecret()));
}
